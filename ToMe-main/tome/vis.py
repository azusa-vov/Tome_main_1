...
def make_visualization(
    ...
    # source: [batch, tokens, patches]
    # vis: [batch, patches]
    # vis assigns patches which are merged together to the same group
    ...
    for i in range(num_groups):
        ...
        # Upsample to the image size [1, 1, H, W]
        mask = F.interpolate(mask, size=(h, w), mode="nearest")
        mask = mask.view(h, w, 1).numpy()

        # Color the mask by the average color of the image in this region
        color = (mask * img).sum(axis=(0, 1)) / mask.sum()
        # Create an eroded edge for better visualization
